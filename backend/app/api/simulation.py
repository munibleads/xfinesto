...
def create_simulation():
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "The project has not built a graph yet. Please call /api/graph/build first."
            }), 400
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    """
    Check whether the simulation has completed preparation.
...
def prepare_simulation():
    ...
    4. LLM intelligently generates simulation config (with retry logic)
    ...
    try:
        ...
        # Check if preparation is already complete (to avoid regeneration)
        ...
        try:
            ...
            # Failure does not affect the rest of the flow; the background task will retry
...
def get_prepare_status():
    ...
    1. Query an ongoing task's progress by task_id
    ...
    try:
        ...
        if not task:
            # Task not found, but if simulation_id is available, check if preparation is complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Iterates the reports directory, finds reports matching the simulation_id,
...
def get_simulation_history():
    ...
    Used for homepage history display; returns an enriched simulation list
    ...
    try:
        ...
        for sim in simulations:
            ...
            if run_state:
                ...
                # Use user-set total_rounds; fall back to recommended_rounds if not set
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    - Suitable for real-time viewing during the generation process
    - Returns extra metadata (e.g., file modification time, whether generation is in progress)
    - Can return partial info even if config hasn't finished generating
    ...
    try:
        ...
        # If config exists, extract some key stats
        if config:
...
def get_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not config:
            return jsonify({
                "success": False,
                "error": f"Simulation config not found. Please call the /prepare endpoint first."
            }), 404
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "max_rounds": 100,                     // optional: maximum simulation rounds, used to cap very long simulations
            "enable_graph_memory_update": false,   // optional: whether to dynamically update Agent activity into Zep graph memory
            "force": false                         // optional: force restart (will stop a running simulation and clear logs)
        }
    ...
    About the force parameter:
        ...
        - Useful for scenarios where you need to re-run a simulation

    About enable_graph_memory_update:
        - When enabled, all Agent activities during the simulation (posting, commenting, liking, etc.)
          are updated in real time to the Zep graph
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                # Preparation complete, check if a process is still running
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # Get current round's actions (recent_actions only shows the latest round)
        ...
        # recent_actions only shows the content of the current latest round on both platforms
...
def interview_agent():
    ...
    (in waiting-for-command mode after completing the simulation loop).
    ...
    try:
        ...
        if platform and platform not in ("twitter", "reddit"):
            return jsonify({
                "success": False,
                "error": "The platform parameter must be 'twitter' or 'reddit'"
            }), 400
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "The simulation environment is not running or has been closed. Please ensure the simulation has completed and is in waiting-for-command mode."
            }), 400
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // required, list of interviews
                ...
                {
                    "agent_id": 1,
                    "prompt": "What do you think about B?"  // no platform specified uses the default
                }
            ],
...
def close_simulation_env():
    ...
    waiting-for-command mode gracefully.

    Note: This is different from the /stop endpoint. /stop forcefully terminates
    the process, while this endpoint lets the simulation gracefully close
