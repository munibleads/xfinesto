...
Dynamically updates agent activities from simulations to Zep graph
"""
...
class AgentActivity:
    ...
    def _describe_create_comment(self) -> str:
        """Create comment - includes comment content and post info being commented on"""
        content = self.action_args.get("content", "")
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
...
class ZepGraphMemoryUpdater:
    ...
    All meaningful behaviors will be updated to Zep, with complete context in action_args:
    - Original post content for liked/disliked posts
    - Original post content for reposted/quoted posts
    - User names for followed/muted users
    - Original comment content for liked/disliked comments
    """
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        # Platform-based activity buffers (each platform accumulates to BATCH_SIZE before batch sending)
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_args will contain complete context information (e.g., original post content, user name, etc.).
