...
Features:
- Does not immediately close the environment after simulation completion, enters command waiting mode
...
class TwitterSimulationRunner:
    ...
    # Twitter available actions (INTERVIEW is not included, INTERVIEW can only be triggered manually via ManualAction)
    AVAILABLE_ACTIONS = [
    ...
    def _create_model(self):
        ...
        Configuration is unified using the project root .env file (highest priority):
        - LLM_API_KEY: API key
        ...
        # If .env doesn't have it, use config as fallback
        if not llm_model:
    ...
    def _get_active_agents_for_round(
        ...
        # Calculate activation probability for each Agent based on their configuration
        candidates = []
...
async def main():
    ...
    # Initialize logging configuration (using fixed file names, cleaning old logs)
