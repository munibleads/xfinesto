...
Log Structure:
    ...
    ├── simulation.log       # Main simulation process logs
...
if sys.platform == 'win32':
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        """
        Wrapper for open() function, defaulting to UTF-8 encoding for text mode
...
# Script is fixed in the backend/scripts/ directory
...
def disable_oasis_logging():
    ...
    OASIS logs are too verbose (logs each agent's observations and actions), we use our own action_logger
    """
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None/not specified: Interview both platforms simultaneously, return combined results
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Default platform (can be overridden by each interview item)
                - "twitter": Only interview Twitter platform
                - "reddit": Only interview Reddit platform
                - None/not specified: Each agent is interviewed on both platforms
        """
        # Group by platform
        twitter_interviews = []
        reddit_interviews = []
...
# Non-core action types to filter (these actions have low analysis value)
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows displaying real entity names in actions.jsonl instead of "Agent_0" style codes
...
def fetch_new_actions_from_db(
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Simplify action_args, keeping only key fields (keep full content, don't truncate)
            simplified_args = {}
...
def _enrich_action_context(
    ...
    try:
        ...
        # Quote post: supplement original post content, author and quote comment
            ...
            # Get quote post's quote content (quote_content)
            if new_post_id:
                cursor.execute("""
        ...
        # Follow user: supplement target user's name
        ...
        # Mute user: supplement target user's name
        ...
        # Create comment: supplement post information being commented on
...
async def run_twitter_simulation(
    ...
    log_info("Creating LLM model...")
    try:
        model = create_model(config, use_boost=False)
        log_info("LLM model created successfully")
    except Exception as e:
        log_info(f"LLM model creation failed: {e}")
        raise
    ...
    log_info(f"Generating Twitter Agent graph (profile: {profile_path})...")
    try:
        ...
        log_info(f"Agent graph generated successfully, total {agent_count} agents")
    except asyncio.TimeoutError:
        log_info("Error: Agent graph generation timed out (5 minutes)")
        raise
    except Exception as e:
        log_info(f"Agent graph generation failed: {e}")
        raise
    
    # Get Agent real name mapping from config file (use entity_name instead of default Agent_X)
    agent_names = get_agent_names_from_config(config)
    # If a certain agent is not in the config, use OASIS default name
    ...
    log_info("Creating Twitter simulation environment...")
    try:
        ...
        log_info("Simulation environment created successfully")
    except Exception as e:
        log_info(f"Simulation environment creation failed: {e}")
        raise
    
    log_info("Resetting simulation environment...")
    try:
        await asyncio.wait_for(result.env.reset(), timeout=60)
        log_info("Environment reset successful")
    except asyncio.TimeoutError:
        log_info("Error: Environment reset timed out (60 seconds)")
        raise
    except Exception as e:
        log_info(f"Environment reset failed: {e}")
        raise
    ...
    for round_num in range(total_rounds):
        ...
        # Offset by 8 hours so simulations starting with few rounds still hit agents' active hours
        simulated_hour = (8 + simulated_minutes // 60) % 24
...
async def run_reddit_simulation(
    ...
    log_info(f"Generating Reddit Agent graph (profile: {profile_path})...")
    try:
        result.agent_graph = await asyncio.wait_for(
            generate_reddit_agent_graph(
                profile_path=profile_path,
    ...
    log_info("Creating Reddit simulation environment...")
    try:
        result.env = oasis.make(
            agent_graph=result.agent_graph,
...
async def main():
    ...
    # Create shutdown event at the beginning of main function to ensure entire program can respond to exit signals
    ...
    log_manager.info("Starting simulation...")
    ...
    try:
        if args.twitter_only:
            ...
            log_manager.info("Starting Twitter and Reddit simulations in parallel...")
            results = await asyncio.gather(
                run_twitter_simulation(config, simulation_dir, twitter_logger, log_manager, args.max_rounds),
                run_reddit_simulation(config, simulation_dir, reddit_logger, log_manager, args.max_rounds),
                return_exceptions=True  # Capture exceptions instead of throwing
            )
            
            # Check results
            if isinstance(results[0], Exception):
                log_manager.error(f"Twitter simulation failed: {results[0]}")
                raise results[0]
            else:
                twitter_result = results[0]
                
            if isinstance(results[1], Exception):
                log_manager.error(f"Reddit simulation failed: {results[1]}")
                raise results[1]
            else:
                reddit_result = results[1]
                
            log_manager.info("Both platform simulations have completed")
    except Exception as e:
        log_manager.error(f"Simulation execution failed: {e}")
        import traceback
        log_manager.error(f"Error details: {traceback.format_exc()}")
        raise
...
def setup_signal_handlers(loop=None):
    ...
    def signal_handler(signum, frame):
        ...
        # If receiving signal repeatedly, then force exit
        else:
            print("Force exit...")
            sys.exit(1)
