        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay briefly before execution, avoid too rapid switching
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
...
/* Use CSS background pattern to create fixed-spaced square grid */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Start positioning from top-left, only expand at bottom when height changes */
