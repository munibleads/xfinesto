...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // maxRounds passed from Step2
  minutesPerRound: {
    type: Number,
...
const doStartSimulation = async () => {
  ...
  // first reset all state to avoid interference from previous simulation
  resetAllState()
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // extra check: if backend hasn't updated runner_status yet, but platform reports completion
...
const checkPlatformsCompleted = (data) => {
  ...
  // determine if platform enabled by actions_count (if count > 0 or running was true)
