...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract statistics - match "Related Predicted Facts: X items" format
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Parse each person's individual selection reason
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // e.g.: - Selected Parent_601（index 0）: As a representative of parent groups...
        if (!headerMatch) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer; copy as default display only if it is non-placeholder text
          ...
          // No platform marker (very old format), use entire answer
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Attempt to extract edge information (if present)
    ...
    // Attempt to extract node information (if present)
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Check whether a given question has dual-platform answers (filter placeholder text)
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual platform toggle button (only shown when there are real dual-platform answers)
...
const renderMarkdown = (content) => {
  ...
  // Remove the leading h2 heading (## xxx) because section titles are already displayed in the outer layer
  ...
  // Process line breaks - double newlines become paragraph breaks, single newlines become <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Clean up cases where <p><br> follows block-level elements (caused by extra blank lines)
  ...
  // Fix numbering for non-continuous ordered lists: when single-item <ol> is separated by paragraph content, keep numbering incremental
...
const extractFinalContent = (response) => {
  ...
  // If content is long and contains markdown format, try to remove thought process and return
